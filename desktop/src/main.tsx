...
function loadStoredSession(): Session | null {
  ...
  try {
    const saved = JSON.parse(localStorage.session) as Session;
    if (!/^[0-9a-f]{64}$/i.test(saved.syncKey || '')) {
...
function App(){
  ...
  async function login(register=false){
    ...
    const publicKey=await nativeInvoke<string>('device_public_key');
    const dev=await api('/devices/register',{serverUrl,accessToken:auth.accessToken},{method:'POST',body:JSON.stringify({name:await nativeInvoke('device_name'),platform:await nativeInvoke('platform'),publicKey})});
